...
function parseAnalysis(text) {
  ...
  for (const line of lines) {
    if (line.startsWith("## ")) {
      ...
      else if (title.includes("SOURCES CONSULTED") || title.includes("SOURCES")) { pillar = "sources"; icon = "📰"; }
...
function extractTheaterRating(text) {
  ...
  if (text.includes("⚠️") && text.includes("MIXED")) return { rating: "mixed", label: "MIXED — Real Tensions + Theater", color: "#ff6b00", icon: "⚠️" };
  return null;
}
...
function SourcesListSection({ content }) {
...
function PillarSection({ section, index }) {
  ...
  const renderContent = (text) => text.split("\n").map((line, i) => {
    ...
    if (line.includes("🎭 POLITICAL THEATER") || line.includes("🔴 GENUINE CRISIS") || line.includes("⚠️ MIXED")) {
      const r = extractTheaterRating(line);
      return r ? <div key={i} style={{ margin: "8px 0" }}><TheaterBadge rating={r} /></div> : null;
    }
    if (line.startsWith("**") && line.includes("**")) {
      const parts = line.split("**");
      return <p key={i} style={{ margin: "4px 0", lineHeight: 1.7, fontSize: 14, color: "#a8b4c8" }}>{parts.map((part, j) => j % 2 === 1 ? <strong key={j} style={{ color: "#d0d8e8" }}>{part}</strong> : part)}</p>;
    }
    if (line.startsWith("- ")) return <div key={i} style={{ display: "flex", gap: 8, margin: "3px 0 3px 8px", lineHeight: 1.7, fontSize: 14, color: "#a8b4c8" }}><span style={{ color: color + "88", flexShrink: 0 }}>▸</span><span>{line.slice(2)}</span></div>;
    return <p key={i} style={{ margin: "4px 0", lineHeight: 1.7, fontSize: 14, color: "#a8b4c8" }}>{line}</p>;
  });

  if (section.pillar === "header") return <div style={{ textAlign: "center", padding: "20px 0 10px", borderBottom: "1px solid rgba(0,212,255,0.15)" }}><h2 style={{ fontSize: 22, fontWeight: 700, color: "#e8ecf4", letterSpacing: 0.5, margin: 0 }}>{section.title}</h2></div>;

  return (
    <div style={{ background: isSummary ? "linear-gradient(135deg, rgba(0,212,255,0.06), rgba(170,85,255,0.04))" : "linear-gradient(135deg, rgba(10,14,28,0.95), rgba(15,20,35,0.9))", border: isSummary ? "1px solid rgba(0,212,255,0.2)" : `1px solid ${color}22`, borderLeft: `3px solid ${color}`, borderRadius: 8, marginBottom: 16, overflow: "hidden", animation: `fadeSlideIn 0.5s ease ${index * 0.1}s both` }}>
      <button onClick={() => setOpen(!open)} style={{ width: "100%", padding: "14px 18px", display: "flex", alignItems: "center", justifyContent: "space-between", background: `linear-gradient(90deg, ${color}08, transparent)`, border: "none", cursor: "pointer" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ fontSize: 20 }}>{section.icon}</span>
          <span style={{ color, fontWeight: 700, fontSize: 14, letterSpacing: 0.8, fontFamily: "'JetBrains Mono', 'Fira Code', monospace" }}>{section.title}</span>
...
function AnalysisCard({ analysis, onDelete }) {
  ...
  return (
    <div style={{ background: "linear-gradient(135deg, #0a0e1c, #0f1525)", border: "1px solid rgba(0,212,255,0.1)", borderRadius: 10, marginBottom: 12, overflow: "hidden" }}>
      ...
      {expanded && <div style={{ padding: "0 18px 18px" }}>{sections.map((s, i) => <PillarSection key={i} section={s} index={i} />)}</div>}
    </div>
  );
}
      ...
      <main style={{ maxWidth: 900, margin: "0 auto", padding: "32px 24px", position: "relative", zIndex: 1 }}>
        {view === "analyze" ? (<>
          ...
          <div style={{ background: "linear-gradient(135deg, #0a0e1c, #0f1525)", border: "1px solid rgba(0,212,255,0.1)", borderRadius: 12, padding: 20, marginBottom: 16 }}>
            <label style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: "#00d4ff88", letterSpacing: 2, fontWeight: 600, display: "block", marginBottom: 10 }}>POLITICAL EVENT / QUESTION</label>
            <textarea value={question} onChange={e => setQuestion(e.target.value)} placeholder="e.g. What are the implications of the latest US-China trade tensions?" rows={3} style={{ width: "100%", background: "rgba(0,0,0,0.3)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 8, padding: "14px 16px", color: "#d0d8e8", fontSize: 15, fontFamily: "'Segoe UI', sans-serif", resize: "vertical", lineHeight: 1.6, boxSizing: "border-box" }} />
          </div>
          ...
          {currentResult && (
            <div ref={resultRef} style={{ animation: "fadeSlideIn 0.5s ease" }}>
              ...
              {sections.map((s, i) => <PillarSection key={i} section={s} index={i} />)}
            </div>
          )}
          ...
          {!currentResult && !loading && (
            <div style={{ textAlign: "center", padding: "60px 20px", color: "#334" }}>
              ...
              <div style={{ fontSize: 13, color: "#2a3344", marginTop: 8, maxWidth: 400, margin: "8px auto 0" }}>Enter a political event or question to run a full 5-pillar analysis with theater detection</div>
            </div>
          )}
        </>) : (
          <div>
